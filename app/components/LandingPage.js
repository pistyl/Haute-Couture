...
function GarmentPreview({ type, poitrine, epaules, hauteur }) {
  ...
  if (type === 'boubou') {
    return (
      <svg viewBox="0 0 200 240" className="w-full h-56 text-brass stroke-current fill-none transition-all duration-300">
        ...
        <path d="M 85 35 C 85 52, 115 52, 115 35" stroke="currentColor" strokeWidth="2" />
        ...
        <path 
          ...
          strokeWidth="1.5" 
        />
        <line x1="100" y1="58" x2="100" y2={`${58 + 90 * scaleY}`} stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 3" />
        
        {/* Sleeve fold/stitch indicators */}
        <path d={`M ${100 - 55 * scaleX} 35 L ${100 - 55 * scaleX} ${35 + 140 * scaleY}`} stroke="currentColor" strokeWidth="1" strokeDasharray="3 3" className="opacity-40" />
        <path d={`M ${100 + 55 * scaleX} 35 L ${100 + 55 * scaleX} ${35 + 140 * scaleY}`} stroke="currentColor" strokeWidth="1" strokeDasharray="3 3" className="opacity-40" />
        
        {/* Measurement arrows */}
        <text x="100" y="15" className="fill-brass text-[9px] font-mono text-center" textAnchor="middle">Épaules: {epaules}cm</text>
        <line x1={`${100 - 75 * scaleX}`} y1="23" x2={`${100 + 75 * scaleX}`} y2="23" stroke="currentColor" strokeWidth="1" />
        <path d={`M ${100 - 75 * scaleX} 20 L ${100 - 75 * scaleX} 26 M ${100 + 75 * scaleX} 20 L ${100 + 75 * scaleX} 26`} stroke="currentColor" strokeWidth="1" />
      </svg>
    );
  }
  ...
  if (type === 'robe') {
    return (
      <svg viewBox="0 0 200 240" className="w-full h-56 text-terracotta stroke-current fill-none transition-all duration-300">
        {/* Grid outline */}
        <path 
        ...
        />
        {/* Collar line */}
        <path d="M 88 35 C 92 45, 108 45, 112 35" stroke="currentColor" strokeWidth="1.5" />
        
        {/* Wax Pattern details */}
        <path d={`M 100 95 Q 92 125, ${100 - 20 * scaleX} ${40 + 160 * scaleY}`} stroke="currentColor" strokeWidth="0.75" strokeDasharray="3 3" className="opacity-60" />
        <path d={`M 100 95 Q 108 125, ${100 + 20 * scaleX} ${40 + 160 * scaleY}`} stroke="currentColor" strokeWidth="0.75" strokeDasharray="3 3" className="opacity-60" />
        
        {/* Measurement indicators */}
        <text x="100" y="15" className="fill-terracotta text-[9px] font-mono text-center" textAnchor="middle">Poitrine: {poitrine}cm</text>
        <line x1={`${100 - 24 * chestScale}`} y1="23" x2={`${100 + 24 * chestScale}`} y2="23" stroke="currentColor" strokeWidth="1" />
      </svg>
    );
  }
  ...
  return (
    <svg viewBox="0 0 200 240" className="w-full h-56 text-thioup-light stroke-current fill-none transition-all duration-300">
      {/* Pattern Grid */}
      <path 
      ...
      />
      {/* Sleeves with cuffs */}
      <path d={`M ${100 - 32 * scaleX} 35 L ${100 - 58 * scaleX} 55 L ${100 - 48 * scaleX} ${55 + 75 * scaleY} L ${100 - 31 * chestScale} 90`} stroke="currentColor" strokeWidth="2" className="fill-thioup/10" />
      <path d={`M ${100 + 32 * scaleX} 35 L ${100 + 58 * scaleX} 55 L ${100 + 48 * scaleX} ${55 + 75 * scaleY} L ${100 + 31 * chestScale} 90`} stroke="currentColor" strokeWidth="2" className="fill-thioup/10" />
      
      {/* Saharienne Collar lapels */}
      <path d="M 84 35 L 100 52 L 116 35 L 100 68 Z" className="fill-thioup-light/15" stroke="currentColor" strokeWidth="1.5" />
      
      {/* Pockets */}
      <rect x={`${100 - 24 * chestScale}`} y="75" width="13" height="16" rx="1" stroke="currentColor" strokeWidth="1" />
      <rect x={`${100 + 11 * chestScale}`} y="75" width="13" height="16" rx="1" stroke="currentColor" strokeWidth="1" />
      <rect x={`${100 - 26 * chestScale}`} y="125" width="16" height="20" rx="1" stroke="currentColor" strokeWidth="1" />
      <rect x={`${100 + 10 * chestScale}`} y="125" width="16" height="20" rx="1" stroke="currentColor" strokeWidth="1" />
      
      {/* Buttons */}
      <line x1="100" y1="68" x2="100" y2={`${35 + 140 * scaleY}`} stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 3" />
      ...
      {/* Label indicator */}
      <text x="100" y="15" className="fill-thioup-light text-[9px] font-mono text-center" textAnchor="middle">Longueur: {hauteur}cm</text>
      <line x1="25" y1="35" x2="25" y2={`${35 + 145 * scaleY}`} stroke="currentColor" strokeWidth="1" />
      <path d={`M 22 35 L 28 35 M 22 ${35 + 145 * scaleY} L 28 ${35 + 145 * scaleY}`} stroke="currentColor" strokeWidth="1" />
    </svg>
  );
}
...
export default function LandingPage({ onStartAuth, onStartRegister }) {
  ...
  const [mockStock, setMockStock] = useState([
    { id: "stk_1", name: "Basin Riche Getzner Blanc", quantity: 6, unit: "mètres", alertThreshold: 10 },
    { id: "stk_2", name: "Wax Hollandais Hirondelle", quantity: 18, unit: "yards", alertThreshold: 12 },
    { id: "stk_3", name: "Tissu Thioup Indigo Artisanal", quantity: 4, unit: "mètres", alertThreshold: 5 }
  ]);
  ...
  const getFabricCalculation = () => {
    ...
    if (selectedGarment === 'boubou') {
      const meters = Math.round((hauteur * 2.8 + epaules * 1.5) / 100 * 10) / 10;
      return {
      ...
      const yards = Math.round((hauteur * 2.0 + poitrine * 0.9) / 100 * 10) / 10;
      return {
      ...
      const meters = Math.round((hauteur * 1.8 + epaules * 2.2) / 100 * 10) / 10;
      return {
  ...
  const handleLeadSubmit = (e) => {
    ...
    setLeadMsg("✨ Invitation envoyée ! Vérifiez votre boîte mail pour démarrer.");
    setLeadEmail("");
    setTimeout(() => {
      setLeadStatus(null);
      setLeadMsg("");
  ...
  return (
    <div className="h-screen overflow-y-auto bg-charcoal text-gray-100 font-sans selection:bg-brass selection:text-charcoal relative overflow-x-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-0">
        <div className="absolute top-10 left-10 w-[500px] h-[500px] rounded-full bg-brass/5 filter blur-3xl animate-pulse" style={{ animationDuration: '8s' }} />
        <div className="absolute bottom-20 right-10 w-[600px] h-[600px] rounded-full bg-rougeSenegal/3 filter blur-3xl animate-pulse" style={{ animationDuration: '12s' }} />
          ...
          <div className="flex items-center gap-3">
            <div className="border-b border-brass pb-1">
              <span className="font-serif text-2xl tracking-wider text-brass font-bold leading-tight flex items-center gap-2">
                <Icon name="scissors" className="w-6 h-6 text-brass rotate-90" />
                Haute Couture
              </span>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-20 text-center lg:text-left flex flex-col lg:flex-row items-center gap-12">
        <div className="flex-1 space-y-6 max-w-2xl">
          ...
          <h1 className="font-serif text-4xl sm:text-5xl lg:text-6xl tracking-tight leading-tight text-white">
            Gérez votre atelier avec <span className="text-brass">art</span> et <span className="text-brass">précision</span>
          </h1>
          <p className="text-gray-400 text-lg leading-relaxed font-serif">
            Haute Couture est la solution SaaS tout-en-un conçue pour les maîtres tailleurs et créateurs de mode d'exception. Centralisez vos fiches de mesures, suivez la fabrication de vos pièces en temps réel, et pilotez vos stocks sans effort.
          </p>
          ...
          <div className="flex items-center justify-center lg:justify-start gap-6 text-xs text-gray-500 font-mono pt-4">
            ...
            <div className="flex items-center gap-1">✔ Configuration immédiate</div>
          </div>
        </div>
        ...
        <div className="flex-1 w-full max-w-md lg:max-w-none relative">
          ...
          <div className="relative border border-charcoal-light bg-charcoal-dark/80 rounded-2xl p-6 shadow-2xl backdrop-blur-md">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-charcoal-light mb-4">
              ...
              <span className="text-[10px] font-mono text-gray-500">haute-couture.app/atelier</span>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded bg-charcoal/60 border border-charcoal-light hover:border-brass/40 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-brass/10 flex items-center justify-center text-brass font-bold text-xs">FD</div>
                  <div className="text-left">
                    <div className="text-xs font-serif font-bold text-white">Fatou Diome</div>
                    <div className="text-[9px] text-gray-500 font-mono">Basin Getzner d'or</div>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-brass/10 border border-brass/30 text-brass">En Couture</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded bg-charcoal/60 border border-charcoal-light hover:border-brass/40 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-brass/10 flex items-center justify-center text-brass font-bold text-xs">AF</div>
                  <div className="text-left">
                    <div className="text-xs font-serif font-bold text-white">Amadou Fall</div>
                    <div className="text-[9px] text-gray-500 font-mono">Grand Boubou 3 pièces</div>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-yellow-500/10 border border-yellow-500/30 text-yellow-500">En Coupe</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded bg-charcoal/60 border border-charcoal-light opacity-50">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-brass/10 flex items-center justify-center text-brass font-bold text-xs">RD</div>
                  <div className="text-left">
                    <div className="text-xs font-serif font-bold text-white">Rokhaya Diallo</div>
                    <div className="text-[9px] text-gray-500 font-mono">Robe Wax Hirondelle</div>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-vertSenegal/10 border border-vertSenegal/30 text-vertSenegal">Livrée</span>
              </div>
            </div>
            ...
            <div className="mt-4 p-3 rounded-lg border border-rougeSenegal/30 bg-rougeSenegal/5 flex items-center justify-between animate-pulse">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono text-gray-400">Alerte : Basin Riche (Stock Bas)</span>
              </div>
              <span className="text-[10px] text-white font-mono font-bold">18m restants</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
            ...
            <p className="text-gray-400 text-sm leading-relaxed max-w-2xl mx-auto">
              Ajustez les mesures en temps réel pour générer une estimation de métrage de tissu, identifier l'artisan idéal de votre atelier, et voir le patron géométrique s'adapter.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch max-w-6xl mx-auto">
            ...
            <div className="lg:col-span-5 p-6 rounded-2xl border border-charcoal-light bg-charcoal-dark/80 flex flex-col justify-between space-y-6">
              <div>
                <h3 className="text-brass font-serif text-lg font-bold mb-4">1. Choisissez le Modèle</h3>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: 'boubou', label: 'Boubou Basin', icon: 'scissors' },
                    { id: 'robe', label: 'Robe Wax', icon: 'clients' },
                    { id: 'saharienne', label: 'Saharienne Thioup', icon: 'orders' }
                  ].map(item => (
                    <button
                      key={item.id}
              ...
              <div className="space-y-4">
                <h3 className="text-brass font-serif text-lg font-bold">2. Ajustez les Mesures (cm)</h3>
                
                {/* Chest Slider */}
                <div className="space-y-1">
                  <div className="flex justify-between text-xs font-mono text-gray-400">
                    <span>Tour de Poitrine</span>
                    <span className="text-white font-bold">{calcMeasurements.poitrine} cm</span>
                  </div>
                  <input
                    type="range"
                    min="80"
                    ...
                    onChange={(e) => handleSliderChange('poitrine', e.target.value)}
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs font-mono text-gray-400">
                    ...
                    <span className="text-white font-bold">{calcMeasurements.epaules} cm</span>
                  </div>
                  <input
                    type="range"
                    min="35"
                    ...
                    onChange={(e) => handleSliderChange('epaules', e.target.value)}
                    className="w-full h-1 bg-charcoal rounded-lg appearance-none cursor-pointer accent-brass"
                  />
                </div>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs font-mono text-gray-400">
                    ...
                    <span className="text-white font-bold">{calcMeasurements.hauteur} cm</span>
                  </div>
                  <input
                    type="range"
                    min="100"
                    ...
                    onChange={(e) => handleSliderChange('hauteur', e.target.value)}
                    className="w-full h-1 bg-charcoal rounded-lg appearance-none cursor-pointer accent-brass"
                  />
                </div>
              </div>
              ...
              <div className="border-t border-charcoal-light pt-4 text-[10px] text-gray-500 font-mono flex items-center gap-1.5">
                ...
                <span>Ces calculs automatisent l'achat du tissu pour vos tailleurs.</span>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 rounded-2xl border-2 border-dashed border-brass/40 bg-charcoal/50 paper-texture p-6 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
              ...
              <div className="flex-1 w-full space-y-4">
                <div className="border-b border-charcoal-light pb-2">
                  <span className="text-[10px] text-gray-500 uppercase font-mono tracking-wider block">Tissu Recommandé</span>
                  <span className="font-serif text-lg font-bold text-white">{calcDetails.fabric}</span>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[9px] text-gray-500 uppercase font-mono block">Métrage requis</span>
                    <span className="text-sm font-bold text-brass font-mono">{calcDetails.quantity}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-gray-500 uppercase font-mono block">Difficulté coupe</span>
                    <span className="text-xs font-bold text-gray-200">{calcDetails.difficulty}</span>
                  </div>
                </div>

                <div className="border-t border-charcoal-light pt-3">
                  <span className="text-[9px] text-gray-500 uppercase font-mono block">Artisan assignable suggéré</span>
                ...
                <div className="p-3 rounded bg-brass/5 border border-brass/20 flex justify-between items-center mt-2">
                  <span className="text-[10px] font-mono text-brass">Prix Estimé Couture :</span>
                  <span className="text-sm font-bold text-white font-mono">{calcDetails.priceEst}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">
              Dites adieu aux carnets de mesures papier et aux stocks mal comptabilisés. Haute Couture structure les tâches quotidiennes de votre atelier.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="p-6 rounded-xl border border-charcoal-light bg-charcoal/50 hover:border-brass/30 transition-all group hover:-translate-y-1 duration-300">
              ...
              <h3 className="font-serif text-lg font-bold text-white mb-2">Fiches Clients & Mesures</h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Enregistrez les fiches anthropométriques détaillées (poitrine, longueur de manche, épaules, tour de cou, etc.) et accédez-y en un clic lors des essayages.
              </p>
            </div>
            ...
            <div className="p-6 rounded-xl border border-charcoal-light bg-charcoal/50 hover:border-brass/30 transition-all group hover:-translate-y-1 duration-300">
              ...
              <h3 className="font-serif text-lg font-bold text-white mb-2">Suivi de Fabrication</h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Suivez la progression de chaque vêtement à travers les étapes de l'atelier : "En coupe", "En couture", "À l'essayage", et "Livré" à vos clients.
              </p>
            </div>
            ...
            <div className="p-6 rounded-xl border border-charcoal-light bg-charcoal/50 hover:border-brass/30 transition-all group hover:-translate-y-1 duration-300">
              ...
              <h3 className="font-serif text-lg font-bold text-white mb-2">Factures & Acomptes</h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Générez des reçus propres avec le calcul automatique du solde restant à payer sur chaque commande d'atelier.
              </p>
            </div>
            ...
            <div className="p-6 rounded-xl border border-charcoal-light bg-charcoal/50 hover:border-brass/30 transition-all group hover:-translate-y-1 duration-300">
              ...
              <h3 className="font-serif text-lg font-bold text-white mb-2">Alerte de Stock Bas</h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Gerez vos tissus (Wax, Basin) et fournitures (fils, boutons). Recevez une alerte visuelle instantanée dès qu'une ressource franchit le seuil critique.
              </p>
            </div>
            ...
            <div className="p-6 rounded-xl border border-charcoal-light bg-charcoal/50 hover:border-brass/30 transition-all group hover:-translate-y-1 duration-300">
              ...
              <h3 className="font-serif text-lg font-bold text-white mb-2">Statistiques & CA</h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Visualisez la santé financière de votre atelier avec des indicateurs de chiffre d'affaires, acomptes encaissés et commandes en cours.
              </p>
            </div>
            ...
            <div className="p-6 rounded-xl border border-charcoal-light bg-charcoal/50 hover:border-brass/30 transition-all group hover:-translate-y-1 duration-300">
              ...
              <h3 className="font-serif text-lg font-bold text-white mb-2">Multi-tenancy Total</h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Chaque artisan dispose de son compte sécurisé avec sa configuration d'atelier, ses propres artisans assignables et ses bases de clients isolées.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="relative z-10 py-24 border-t border-charcoal-light bg-charcoal/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">
              Explorez ci-dessous comment l'interface s'anime au quotidien dans votre travail de création.
            </p>
          </div>
          ...
          <div className="max-w-5xl mx-auto border border-charcoal-light bg-charcoal-dark/90 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="p-6 sm:p-8 min-h-[320px] flex flex-col justify-between">
              ...
              {activeWalkthroughTab === "dashboard" && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-serif text-base font-bold text-white">Chiffres de l'Atelier Baobab</h4>
                      <p className="text-xs text-gray-500">Données en direct de ce mois</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-charcoal p-4 rounded-xl border border-charcoal-light">
                      <span className="text-[10px] text-gray-500 uppercase font-mono block">Chiffre d'Affaires</span>
                      <span className="text-xl font-bold font-mono text-brass">355 000 FCFA</span>
                    </div>
                    <div className="bg-charcoal p-4 rounded-xl border border-charcoal-light">
                      <span className="text-[10px] text-gray-500 uppercase font-mono block">Acomptes Encaissés</span>
                      <span className="text-xl font-bold font-mono text-vertSenegal">195 000 FCFA</span>
                    </div>
                    <div className="bg-charcoal p-4 rounded-xl border border-charcoal-light">
                      <span className="text-[10px] text-gray-500 uppercase font-mono block">Soldes à Récupérer</span>
                      <span className="text-xl font-bold font-mono text-rougeSenegal">160 000 FCFA</span>
                    </div>
                  </div>

                  {/* Simulated Activity Feed */}
                  <div className="bg-charcoal/50 p-4 rounded-xl border border-charcoal-light">
                    <h5 className="text-xs font-bold text-gray-300 mb-2.5">Activités Récentes</h5>
                    <div className="space-y-2 text-xs">
                      <div className="flex items-center justify-between text-gray-400">
                        ...
                        <span className="text-[10px] font-mono text-gray-600">il y a 10 min</span>
                      </div>
                      <div className="flex items-center justify-between text-gray-400">
                        <span className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-mono text-gray-600">il y a 2h</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {activeWalkthroughTab === "clients" && (
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                  ...
                  <div className="md:col-span-5 space-y-2.5 text-left">
                    <h4 className="font-serif text-sm font-bold text-white mb-2">Sélectionnez un profil</h4>
                    {[
                      { id: "cli_1", name: "Fatou Diome", orders: "1 commande" },
                      { id: "cli_2", name: "Amadou Fall", orders: "2 commandes" }
                    ].map(cli => (
                      <button
                        key={cli.id}
                      ...
                      >
                        ...
                        <span className="text-[10px] font-mono">{cli.orders}</span>
                      </button>
                    ))}
                  </div>
                    ...
                    {selectedMockClient === "cli_1" ? (
                      <div className="space-y-4">
                        <div className="flex justify-between border-b border-brass/25 pb-2">
                          <div>
                            <h4 className="font-serif font-bold text-white text-sm">Fiche de Mesures : Fatou Diome</h4>
                            <p className="text-[10px] text-gray-500">+221 77 123 45 67</p>
                          </div>
                          <span className="text-[10px] font-serif italic text-brass">Dernière mise à jour : hier</span>
                        </div>
                        <div className="grid grid-cols-3 gap-y-3 gap-x-2 text-xs font-mono">
                          <div>
                            <span className="text-[9px] text-gray-500 block">Poitrine</span>
                            <span className="text-white font-bold">98 cm</span>
                          </div>
                          <div>
                            <span className="text-[9px] text-gray-500 block">Taille</span>
                            <span className="text-white font-bold">76 cm</span>
                          </div>
                          <div>
                            <span className="text-[9px] text-gray-500 block">Hanches</span>
                            <span className="text-white font-bold">108 cm</span>
                          </div>
                          <div>
                            <span className="text-[9px] text-gray-500 block">Épaules</span>
                            <span className="text-white font-bold">42 cm</span>
                          </div>
                          <div>
                            <span className="text-[9px] text-gray-500 block">Manches</span>
                            <span className="text-white font-bold">60 cm</span>
                          </div>
                          <div>
                            <span className="text-[9px] text-gray-500 block">Longueur</span>
                        ...
                        <div className="bg-charcoal/60 p-2.5 rounded border border-charcoal-light text-[10px] text-gray-400 leading-relaxed italic">
                          "Préfère les broderies d'or complexes le long du plastron. Basin riche Getzner blanc uniquement."
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="flex justify-between border-b border-brass/25 pb-2">
                          <div>
                            <h4 className="font-serif font-bold text-white text-sm">Fiche de Mesures : Amadou Fall</h4>
                            <p className="text-[10px] text-gray-500">+221 78 987 65 43</p>
                          </div>
                          <span className="text-[10px] font-serif italic text-brass">Dernière mise à jour : il y a 5 jours</span>
                        </div>
                        <div className="grid grid-cols-3 gap-y-3 gap-x-2 text-xs font-mono">
                          <div>
                            <span className="text-[9px] text-gray-500 block">Poitrine</span>
                          ...
                          <div>
                            <span className="text-[9px] text-gray-500 block">Tour Cou</span>
                            <span className="text-white font-bold">44 cm</span>
                          </div>
                          <div>
                            <span className="text-[9px] text-gray-500 block">Pantalon</span>
                            <span className="text-white font-bold">104 cm</span>
                          </div>
                        </div>
                        <div className="bg-charcoal/60 p-2.5 rounded border border-charcoal-light text-[10px] text-gray-400 leading-relaxed italic">
                          "Demande un grand boubou classique 3 pièces très ample. Teinture indigo thioup artisanale uniquement."
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {activeWalkthroughTab === "stock" && (
                <div className="space-y-6">
                  <div className="text-left">
                    <h4 className="font-serif text-base font-bold text-white">Inventaire & Alertes Seuils</h4>
                    <p className="text-xs text-gray-500">Cliquez sur +/- pour simuler les entrées/sorties de l'atelier et tester le déclenchement des alertes.</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    {mockStock.map(item => {
                      ...
                      return (
                        <div key={item.id} className="flex items-center justify-between p-3 bg-charcoal rounded-xl border border-charcoal-light">
                          <div className="flex-1 min-w-0 pr-4 text-left">
                            <span className="text-xs font-serif font-bold text-white block truncate">{item.name}</span>
                            <span className="text-[9px] text-gray-500 font-mono">
                              Seuil critique d'alerte : {item.alertThreshold} {item.unit}
                            </span>
                          </div>
      ...
      <section className="relative z-10 py-24 border-t border-charcoal-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
            <h2 className="font-serif text-3xl sm:text-4xl text-white">Des abonnements simples, adaptés à votre croissance</h2>
            <p className="text-gray-400 text-sm">Démarrez gratuitement pour tester les fonctionnalités, puis choisissez l'abonnement qui vous correspond.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch max-w-5xl mx-auto">
            ...
            <div className="border border-charcoal-light bg-charcoal/40 rounded-2xl p-8 flex flex-col justify-between hover:border-brass/20 transition-all">
              <div className="space-y-6 text-left">
                <div>
                  <h3 className="text-lg font-serif font-bold text-white">Essai Gratuit</h3>
                  <p className="text-xs text-gray-500 font-mono mt-1">Idéal pour démarrer</p>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-serif font-bold text-white">0 F</span>
                  <span className="text-xs text-gray-500 font-mono">/ 14 jours</span>
                </div>
                <ul className="space-y-3 text-xs text-gray-400 font-sans border-t border-charcoal-light pt-6">
                  <li className="flex items-center gap-2">✔ Accès à toutes les fonctionnalités</li>
                  <li className="flex items-center gap-2">✔ Fiches clients et mesures illimitées</li>
                  <li className="flex items-center gap-2">✔ Alertes de stock activées</li>
                  <li className="flex items-center gap-2">✔ Pas de carte de crédit requise</li>
                </ul>
              </div>
              <button 
                onClick={onStartRegister} 
            ...
            <div className="border border-brass bg-charcoal-dark/90 rounded-2xl p-8 flex flex-col justify-between shadow-2xl relative hover:scale-102 transition-all">
              ...
              <div className="space-y-6 text-left">
                <div>
                  <h3 className="text-lg font-serif font-bold text-brass">Plan Mensuel</h3>
                  <p className="text-xs text-brass/75 font-mono mt-1">Pour les ateliers actifs</p>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-serif font-bold text-white">10 000 F</span>
                  <span className="text-xs text-gray-500 font-mono">/ mois</span>
                </div>
                <ul className="space-y-3 text-xs text-gray-300 font-sans border-t border-charcoal-light pt-6">
                  <li className="flex items-center gap-2 text-brass">✔ Tout ce qui est dans l'essai gratuit</li>
                  <li className="flex items-center gap-2">✔ Base de données Supabase persistante</li>
                  <li className="flex items-center gap-2">✔ Export et imports des données (Backup JSON)</li>
                  <li className="flex items-center gap-2">✔ Support réactif 24/7</li>
                </ul>
              </div>
              <button 
                onClick={onStartRegister} 
            ...
            <div className="border border-charcoal-light bg-charcoal/40 rounded-2xl p-8 flex flex-col justify-between hover:border-brass/20 transition-all">
              <div className="space-y-6 text-left">
                <div>
                  <h3 className="text-lg font-serif font-bold text-white">Plan Trimestriel</h3>
                  <p className="text-xs text-gray-500 font-mono mt-1">Économisez 17%</p>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-serif font-bold text-white">25 000 F</span>
                  <span className="text-xs text-gray-500 font-mono">/ 3 mois</span>
                </div>
                <ul className="space-y-3 text-xs text-gray-400 font-sans border-t border-charcoal-light pt-6">
                  <li className="flex items-center gap-2">✔ Avantages du plan mensuel</li>
                  <li className="flex items-center gap-2 text-brass">✔ 5 000 F d'économie sur 3 mois</li>
                  <li className="flex items-center gap-2">✔ Idéal pour stabiliser votre budget</li>
      ...
      <section className="relative z-10 py-24 border-t border-charcoal-light bg-charcoal-dark/20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 space-y-4">
            ...
            <p className="text-gray-400 text-sm">Tout ce que vous devez savoir pour démarrer l'esprit tranquille.</p>
          </div>
          ...
          <div className="space-y-4 text-left">
            {[
              ...
              },
              {
                ...
                a: "Oui, absolument. Nous croyons en la souveraineté de vos données. Depuis la section de configuration de l'atelier, vous pouvez à tout moment exporter l'intégralité de vos bases de données clients, mesures et commandes dans un fichier JSON structuré standard."
              },
              {
              ...
              },
              {
                ...
                a: "Chaque atelier enregistré dispose d'un espace totalement étanche. Toutes les communications sont chiffrées en HTTPS et les données sont hébergées de manière sécurisée en Europe sur des clusters PostgreSQL gérés par Supabase, avec des sauvegardes quotidiennes automatiques."
              }
            ].map((faq, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`transition-all duration-300 ease-in-out overflow-hidden ${
                    isOpen ? 'max-h-[200px] border-t border-charcoal-light bg-charcoal-dark/20' : 'max-h-0'
                  }`}>
      ...
      <section className="relative z-10 py-24 border-t border-charcoal-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 space-y-4">
            <h2 className="font-serif text-3xl text-white">Paroles de Couturiers</h2>
            <p className="text-gray-400 text-sm">Ils utilisent notre logiciel au quotidien pour piloter leur maison de couture.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto text-left">
            {[
              {
                quote: "Depuis que nous utilisons Haute Couture, nous ne perdons plus aucun historique. Les mesures des clients sont archivées de manière structurée. Plus besoin de feuilleter nos vieux cahiers pendant des heures !",
              ...
              {
                quote: "Le suivi d'avancement par étape (Coupe, Couture, Essayage, Livré) nous a permis d'optimiser le planning de nos tailleurs et brodeurs. Le retard de livraison a été réduit à zéro, pour le plus grand bonheur de nos clients.",
              ...
              {
                quote: "La fonctionnalité d'alertes de stock de tissus est exceptionnelle. Plus de mauvaise surprise où le Wax commandé par un client est en rupture dans nos tiroirs. Un outil vraiment moderne et adapté à nos réalités.",
                name: "Awa Diallo",
              ...
              >
                ...
                <div className="border-t border-charcoal-light mt-6 pt-4 flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-serif font-bold text-white">{t.name}</h4>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative rounded-3xl border border-brass/40 bg-gradient-to-br from-charcoal-dark via-charcoal to-charcoal-light/40 p-8 sm:p-12 shadow-2xl overflow-hidden text-center space-y-6">
            ...
            <p className="text-gray-400 text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
              Rejoignez des dizaines d'artisans d'exception qui modernisent la gestion de leur atelier de couture dès aujourd'hui.
            </p>

            <form onSubmit={handleLeadSubmit} className="max-w-md mx-auto flex flex-col sm:flex-row items-center gap-3 pt-4">
              <input
                type="email"
            ...
            {leadStatus && (
              <div className={`text-xs font-mono font-medium max-w-md mx-auto transition-opacity duration-300 ${
                leadStatus === "success" ? "text-vertSenegal" : "text-rougeSenegal"
              }`}>
                {leadMsg}
              </div>
            )}
      ...
      <footer className="relative z-10 border-t border-charcoal-light bg-charcoal-dark/50 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">
          ...
          <p className="text-[10px] text-gray-600 font-mono">&copy; 2026 Haute Couture. Tous droits réservés.</p>
        </div>
      </footer>
    </div>
  );
